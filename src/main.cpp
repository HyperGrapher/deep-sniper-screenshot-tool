...
constexpr wchar_t kAppName[] = L"Deep Sniper";
constexpr wchar_t kAppId[] = L"DeepSniper";
constexpr wchar_t kTrayWindowClass[] = L"DeepSniper.TrayWindow";
...
private:
    ...
    void buildUi() {
        ...
        addLabel(20, 0, 350, kHeaderHeight, "Deep Sniper", 16, kText, FL_HELVETICA_BOLD);
    ...
    void saveDisplayName() {
        try {
            ...
            state_.displayName = value.empty() ? "Deep Sniper" : value;
            displayNameInput_->value(state_.displayName.c_str());
            persistState();
            updateStateLabels();
...
LRESULT TrayIcon::handleMessage(HWND window, UINT message, WPARAM wordParameter, LPARAM longParameter) {
    ...
    if (message == kTrayMessage) {
        ...
        if (longParameter == WM_RBUTTONUP || longParameter == WM_CONTEXTMENU) {
            ...
            AppendMenuW(menu, MF_STRING, kTrayOpenCommand, L"Open Deep Sniper");
